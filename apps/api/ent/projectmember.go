...
func (*ProjectMember) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case projectmember.FieldIsActive:
			values[i] = new(sql.NullBool)
		case projectmember.FieldRole:
			values[i] = new(sql.NullInt64)
		case projectmember.FieldCreatedAt, projectmember.FieldUpdatedAt, projectmember.FieldDeletedAt:
			values[i] = new(sql.NullTime)
		case projectmember.FieldID, projectmember.FieldWorkspaceID, projectmember.FieldProjectID, projectmember.FieldUserID:
...
func (_m *ProjectMember) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case projectmember.FieldWorkspaceID:
			if value, ok := values[i].(*uuid.UUID); !ok {
