...
func (*Reaction) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case reaction.FieldWorkItemID, reaction.FieldCommentID:
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		case reaction.FieldEmoji:
			values[i] = new(sql.NullString)
		case reaction.FieldCreatedAt, reaction.FieldUpdatedAt, reaction.FieldDeletedAt:
			values[i] = new(sql.NullTime)
		case reaction.FieldID, reaction.FieldWorkspaceID, reaction.FieldProjectID, reaction.FieldUserID:
...
func (_m *Reaction) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case reaction.FieldWorkspaceID:
			if value, ok := values[i].(*uuid.UUID); !ok {
				return fmt.Errorf("unexpected type %T for field workspace_id", values[i])
			} else if value != nil {
				_m.WorkspaceID = *value
			}
		case reaction.FieldProjectID:
			if value, ok := values[i].(*uuid.UUID); !ok {
				return fmt.Errorf("unexpected type %T for field project_id", values[i])
			} else if value != nil {
				_m.ProjectID = *value
			}
		case reaction.FieldWorkItemID:
